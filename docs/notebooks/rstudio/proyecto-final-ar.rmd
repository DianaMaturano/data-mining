...
Escribe la fecha actual (**NOTE-01**): 31 de mayo de 2021
...
```{r}
NOMBRE_COMPLETO = "Diana Aguilera Maturano"
DIGITOS_MATRICULA = 01745249
```
...
```{r}
try(
  plot(smalltx, main = paste0("Elaborado por: ", "Diana Aguilera Maturano", " (", 01745249, ")" ))  
)
...
```{r}
try(
  itemFrequencyPlot(smalltx, topN=10, main = paste0("Elaborado por: ", "Diana Aguilera Maturano", " (", 01745249, ")" ))
)
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**): No hay relación con otro tipo de producto
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): Porque las métricas de support y confidence exluyen a la regla 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Se enfoca en distinguir en el número de sesiones del data set que inicia el usuario para modelado de escucha, interacciones en streaming, recuperación de información musical (MIR, por sus siglas en inglés) y recomendaciones
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):se distingue las canciones que escucha el usuario
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí porque podemos ver el comportamiento que el usuario tiene al escuchar una canción y se pueden hacer diferentes asociaciones con diferentes datasets y diferentes análisis como el hecho de que se hayan saltado 3 cancion, puede dejar la 4 canción. Inspecciona la estructura de y, el dataset preparado.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): ID: t_bacf06d3-9185-4183-84ea-ff0db51475ce Frecuencia: 885
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 20
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Que los usuarios crean alrededor de 14.65 sesiones de streaming en promedio.
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**):Ninguna de las reglas de asociación cumplen con los parámetros establecidos bajo los valores de support y confidence
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`?: Se crean muchas reglas bajando los valores de support y confidence y esto ya no lo hace tna relevante en cuanto a la relación de las reglas.
...
```{r}
try(
  plot(rules, main=paste0("Elaborado por: ", "Diana Aguilera Maturano", " (", 01745249, ")" ))
)
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/k4HNQxlyuXw
